...
$('.portfolio-navigation .next-page').addClass("disable-fix");
    ...
    function sticky_header() {
        ...
        $('body:not(.inner-page) .section').waypoint({
            handler: function(direction) {
                ...
                if ($active_section[0].id == ""){
                    $active_section[0].id = "<span class='red'>Rien a changé ici !</span>"
                $('#sectionName').html($active_section[0].id);
                }else {
                $('#sectionName').html($active_section[0].id);
                $('#backgroundcolor,.motif-container').attr('data-pour',$active_section[0].id);
                }
            },
            offset: '15%'
        });
  ...
  // form sent successfully and without errors
                    $('#contact-form').fadeOut(70, function() {
                        var $this = $(this);
                        setTimeout(function() {
                            $this.fadeIn();
                        }, 6000);
